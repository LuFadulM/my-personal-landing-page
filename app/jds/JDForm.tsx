...
export default function JDForm({ jd }: Props) {
  ...
  async function handleGenerate() {
    ...
    setGenerating(true);
    try {
      const text = await generateJD({ title, company, seniority });
      setDescription(text);
    } finally {
      setGenerating(false);
    }
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    setSaving(true);
    if (jd) {
      jdStore.update(jd.id, { title, company, seniority, description });
    } else {
      createJDWithAutomation({ title, company, seniority, description });
    }
    router.push('/jds');
    router.refresh();
  }
